...
def main():
    ...
    while True:
        ...
        if len(lmList) != 0:
            ...
            # change audo to said percentage 
            audio.change_volume(percent)

            # present current volume on screen
            cv2.putText(img, str(audio.get_volume()), (10,70), cv2.FONT_HERSHEY_PLAIN, 3, (0, 255, 0), 3)
...
def convert_to_percent(distance):
    ...
    # turn the distance into a percentage 0-100 so it can be used to change the volume 
    # return percentage 
